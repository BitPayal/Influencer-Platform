...
const InfluencerLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(email, password, 'influencer');
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center relative overflow-hidden bg-gradient-to-br from-orange-50 via-white to-amber-50">
      <Head>
        <title>Influencer Login - Cehpoint</title>
      </Head>
        ...
        <div className="text-center mb-8">
            <Link href="/">
                <h1 className="text-4xl font-extrabold cursor-pointer tracking-tight">
                    <span className="text-orange-600">Cehpoint</span> <span className="text-gray-900">Creators</span>
                </h1>
            </Link>
        ...
        <div className="bg-white/80 backdrop-blur-lg py-10 px-8 shadow-2xl rounded-2xl border border-white/50">
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-gray-600">
              Not joined yet?{' '}
              <Link href="/register/influencer" className="font-semibold text-orange-600 hover:text-orange-500 transition-colors">
                Register as Influencer here
              </Link>
            </p>
          </div>
        </div>
